...
// array of svg icons
const weatherIcons = {
	rain: `<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
...
<path d="M4.64,16.91c0-1.15,0.36-2.17,1.08-3.07c0.72-0.9,1.63-1.47,2.73-1.73c0.31-1.36,1.02-2.48,2.11-3.36s2.34-1.31,3.75-1.31
	...
	'clear-day': `<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	viewBox="0 0 30 30" style="enable-background:new 0 0 30 30;" xml:space="preserve">
...
c-0.24,0-0.44,0.08-0.62,0.25S24.67,11.37,24.67,11.6z"/>
	...
	'partly-cloudy-day': `<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	viewBox="0 0 30 30" style="enable-background:new 0 0 30 30;" xml:space="preserve">
...
z"/>
	...
	cloudy: `<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	viewBox="0 0 30 30" style="enable-background:new 0 0 30 30;" xml:space="preserve">
<path d="M3.89,17.6c0-0.99,0.31-1.88,0.93-2.65s1.41-1.27,2.38-1.49c0.26-1.17,0.85-2.14,1.78-2.88c0.93-0.75,2-1.12,3.22-1.12
c1.18,0,2.24,0.36,3.16,1.09c0.93,0.73,1.53,1.66,1.8,2.8h0.27c1.18,0,2.18,0.41,3.01,1.24s1.25,1.83,1.25,3
c0,1.18-0.42,2.18-1.25,3.01s-1.83,1.25-3.01,1.25H8.16c-0.58,0-1.13-0.11-1.65-0.34S5.52,21,5.14,20.62
...
z"/>
	...
	'partly-cloudy-night': `<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	viewBox="0 0 30 30" style="enable-background:new 0 0 30 30;" xml:space="preserve">
<path d="M4.14,16.9c0-1.16,0.35-2.18,1.06-3.08s1.62-1.47,2.74-1.72c0.23-1.03,0.7-1.93,1.4-2.7c0.7-0.77,1.55-1.32,2.53-1.65
   ...
   c-0.58-0.01-1.15,0.13-1.7,0.44C18.38,7.82,17.93,8.24,17.6,8.79z"/>
	...
	'clear-night': `<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
 viewBox="0 0 30 30" style="enable-background:new 0 0 30 30;" xml:space="preserve">
...
// handles fetch request to server
async function getWeather(zipCode) {
    try {
        const response = await fetch(`https://weather.snipectrl.com/weather?zip=${zipCode}`);
...
// handles 
function setIconClass(iconType, customClass) {
	const svgIcon = weatherIcons[iconType];
	if (!svgIcon) {
		console.error(`Icon type ${iconType} not found.`);
		return '';
	};
	return svgIcon.replace('<svg ', `<svg class="${customClass}" `);
}
...
function updateWeatherUI(data) {
	...
	mainTempIcon.innerHTML = setIconClass('.temp-svg', weatherIcons[data.icon])
	...
	let moonIcon = document.getElementById('moon-phase-icon');
...
<path d="M3.74,14.44c0,2.04,0.5,3.93,1.51,5.65s2.37,3.1,4.1,4.1s3.61,1.51,5.65,1.51s3.92-0.5,5.65-1.51s3.09-2.37,4.09-4.1
	...
	hourElements.forEach((hourElement, index) => {
		let hour = hours + index + 1;
		if (hour > 23) {
			hour = hour - 24;
		}
		const timePeriod = hour >= 12 ? 'PM' : 'AM';
		const formattedHour = hour % 12 || 12;
		const dataHourIcon = data.hours[hour].icon

		hourElement.textContent = `${formattedHour}:00 ${timePeriod}`;
		hourTempElements[index].textContent = `${data.hours[hour].temp}°`;
		hourIcons[index].innerHTML = `${weatherIcons[dataHourIcon]}`;
		setIconClass(dataHourIcon, '.stats-icon');
	})
};
...
document.addEventListener('DOMContentLoaded', async () => {
    const defaultZipCode = '97338';
    const data = await getWeather(defaultZipCode);

    if (!data) {
        console.log('No data returned for the default zip code.');
        alert('Failed to fetch weather data for the default zip code.');
        return;
    }
